...
def _dhash(p): return _hl.sha256(("medisense_vit_2024"+p).encode()).hexdigest()
DEMO_DOCTOR = {"username":"doctor","password_hash":_dhash("doc123"),"name":"Dr. Priya Sharma"}
    ...
    if not st.session_state.get("doctor_logged_in"):
        ...
        with col:
            ...
            if st.button("🔐 Login as Doctor", use_container_width=True):
                if doc_user == DEMO_DOCTOR["username"] and _dhash(doc_pass) == DEMO_DOCTOR["password_hash"]:
