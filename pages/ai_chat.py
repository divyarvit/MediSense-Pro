...
def call_groq(messages):
    ...
    if not groq_key:
        return None, "GROQ_KEY not found in .streamlit/secrets.toml"
    try:
...
def show():
    ...
    if not _get_groq_key():
        st.error("⚠️ Groq API key not configured. Add GROQ_KEY to .streamlit/secrets.toml")
        return
