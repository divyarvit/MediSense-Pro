...
try:
    ...
    pass  # Model load error shown at runtime when user tries to predict
...
| Thalassemia | 0=Normal, 1=Fixed Defect, 2=Reversible Defect | Blood oxygen-carrying disorder |
        ...
        if st.button("🔍 Run Clinical Heart Disease Assessment", use_container_width=True):
            ...
            inp   = (raw - means) / stds
            if MODEL is None:
                st.error("❌ Heart model failed to load. Please check models/heart_disease_model1.sav exists.")
                st.stop()
