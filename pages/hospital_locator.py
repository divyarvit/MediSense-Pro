...
def show():
    ...
    c1, c2 = st.columns(2)
    with c1:
        _user_city = st.session_state.get("city", "")
        _city_list = sorted(HOSPITALS.keys())
        _default_idx = 0
        if _user_city:
            for _i, _c in enumerate(_city_list):
                if _user_city.lower() in _c.lower() or _c.lower() in _user_city.lower():
                    _default_idx = _i
                    break
        city = st.selectbox("📍 Your City", _city_list,
                            index=_default_idx,
                             index=0)
    with c2:
