...
def _gemini_url():
    key = _get_gemini_key()
    return (f"https://generativelanguage.googleapis.com/v1/models/"
...
def analyse_with_gemini(image_bytes, category_desc, patient_description, duration):
    ...
    req = urllib.request.Request(_gemini_url(), data=payload,
...
def show():
    ...
    if not _get_gemini_key():
        st.error("⚠️ Gemini API key not configured. Add GEMINI_KEY to .streamlit/secrets.toml")
        return
    ...
    with col_upload:
        ...
        category_name = st.selectbox("🏷️ Type of symptom", list(CATEGORIES.keys()))
