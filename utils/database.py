...
def hash_pw(pw):
    """SHA-256 + static salt — stronger than plain SHA-256."""
    salt = "medisense_vit_2024"
...
def init_db():
    ...
    for col_sql in [
        "ALTER TABLE reports ADD COLUMN numeric_value REAL",
        "ALTER TABLE reports ADD COLUMN doctor_note TEXT",
        "ALTER TABLE reports ADD COLUMN family_member_id INTEGER DEFAULT NULL",
        "ALTER TABLE reports ADD COLUMN family_member_name TEXT DEFAULT NULL",
    ]:
        try:
            c.execute(col_sql)
        except Exception:
...
def _check_and_create_alerts(c, user_id, module, severity, diagnosis, numeric_value):
    ...
    if "Heart" in module and severity in ["Severe", "Moderate"]:
        alerts.append(("health", "❤️ Heart Disease risk detected. Schedule a cardiologist appointment this week."))
    if "Blood Pressure" in diagnosis and severity != "Mild":
        alerts.append(("health", "🩺 Hypertension risk noted. Monitor your BP daily and reduce salt intake."))
...
# ─── FAMILY HEALTH VAULT ──────────────────────────────────────────────────────
def _ensure_family_table(c, conn):
